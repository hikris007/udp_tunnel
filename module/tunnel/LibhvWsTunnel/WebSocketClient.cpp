...
WebSocketClient::WebSocketClient(hv::EventLoopPtr loop)
    : TcpClientTmpl<hv::WebSocketChannel>(loop)
{
    state = WS_CLOSED;
...
int WebSocketClient::open(const std::string &url, const http_headers &headers) {
...
int WebSocketClient::open(const char* _url, const http_headers& headers) {
    ...
    onMessage = [this](const WebSocketChannelPtr& channel, hv::Buffer* buf) {
    ...
    state = CONNECTING;
    this->loop()->runInLoop(std::bind(&TcpClientEventLoopTmpl::startConnect, this));
    return 0;
}
